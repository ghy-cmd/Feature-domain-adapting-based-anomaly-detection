...
def create_storage_folder(results_path, log_project, log_group):
    '''
    :param results_path: 存储路径/home/guihaoyue_bishe/res_test
    :param log_project: 工程名MVTecAD_Results
    :param log_group: 组名test_res
    :return:
    '''
    os.makedirs(results_path, exist_ok=True)  # exist_ok为TRUE允许文件夹存在
    project_path = os.path.join(results_path, log_project)  # /home/guihaoyue_bishe/res/MVTecAD_Results
    os.makedirs(project_path, exist_ok=True)
    # /home/guihaoyue_bishe/res_test/MVTecAD_Results/IM224_WR50_L2-3_P01_D1024-1024_PS-3_AN-1_S0_i
    save_path = os.path.join(project_path, log_group)
    counter = 0
    while os.path.exists(save_path):
        save_path = os.path.join(project_path, log_group + "_" + str(counter))
        counter += 1
...
def plot_res_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        axes[0].imshow(image.transpose(1, 2, 0))
        axes[1].imshow(mask.transpose(1, 2, 0))
        segmented_image = np.zeros_like(segmentation, dtype=np.uint8)
        segmented_image[segmentation > optimal_threshold] = 255
        # 将二值图像转换为PIL图像
        pil_image = PIL.Image.fromarray(segmented_image.astype(np.uint8))
        # 将PIL图像转换为三通道的黑白图像

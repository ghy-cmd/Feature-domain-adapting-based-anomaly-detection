...
from patchcore.attention import Attention
...
class PatchCore(torch.nn.Module):
    ...
    def load(
        ...
        ATTENTION机制
        '''
        self.ATTENTION = Attention(1024, 1024).to(device)
        logging.basicConfig(level=logging.INFO)
        LOGGER = logging.getLogger("optimizer")
        '''
        optim
        '''
        self.param_list = [{'params': self.ATTENTION.parameters(), 'lr': 5e-4}]
    ...
    def _attention_train(self, train_loader, feature_bank):
        ...
        for epoch in range(self.epochs):
            ...
            torch.cuda.empty_cache()
            with tqdm.tqdm(
                    train_loader, desc="Training attention model...", position=1, leave=False
            ) as data_iterator:
                for image in data_iterator:
                    if isinstance(image, dict):
                        image = image["image"]  # 对应之前迭代的字典
                    ...
                    z_aug, z_aug_new, z, z_new = self.ATTENTION(new_z_list, z_tensor)

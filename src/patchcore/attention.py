...
class Attention_module(nn.Module):
    def __init__(self, fea_dim, out_dim):
        ...
        self.WQ = Parameter(torch.Tensor(self.fea_dim, self.out_dim))  # torch.size[1024,1024]
        self.WK = Parameter(torch.Tensor(self.fea_dim, self.out_dim))  # torch.size[1024,1024]
        self.WV = Parameter(torch.Tensor(self.fea_dim, self.fea_dim))  # torch.size[1024,1024]

        self.reset_parameters()

    def reset_parameters(self):
        nn.init.xavier_uniform_(self.WQ)  # 初始化
